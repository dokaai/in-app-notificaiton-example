...
import { connectHostSocket } from "@/lib/socket-client";
...
export function LoginScreen() {
  ...
  const initializeSession = useCallback(async (auth: {
    ...
    try {
      const tokenResponse = await requestCustomerJwt({
      ...
      const jwtToken = tokenResponse.jwtToken;
      updateStep("customer", "success");
      toast.success(getApiSuccessMessage(tokenResponse, "Customer JWT minted successfully."));
